...
import {actionToCreateRoom, actionToRemoveUserFromRoom} from "./actions/UserAction";
...
export const CallProvider = ({ children }) => {
  ...
  const { isSupported, released, request, release } = useWakeLock({
    ...
    const createToken = async (roomName) => {
        ...
        const response = await fetch(
            ...
            `${
                process.env.REACT_APP_NETLIFY_URL || "https://letzconnects.daily.co"
  ...
  const joinRoom = useCallback(
    async ({ userName, name, moderator,members,setCallLoading ,setRequestToStartConCall}) => {
      if (callFrame) {
        callFrame.leave();
      }
      let roomInfo = { name };
      ...
      const options = {
        ...
        url: `${
          process.env.REACT_APP_DAILY_DOMAIN || "https://letzconnects.daily.co"
        }/${roomInfo?.name}`,
        userName,
      };
      if (roomInfo?.token) {
      ...
      await call
        ...
        .then(() => {
          ...
          if(setCallLoading != undefined && setCallLoading){
            setCallLoading(false);
          }

          if(setRequestToStartConCall != undefined && setRequestToStartConCall){
  ...
  const removeFromCall = useCallback(
    (participant) => {
      ...
      callFrame.sendAppMessage({ msg: FORCE_EJECT }, participant?.session_id);
        ...
        let splitArray = participant?.user_name?.split('_');
        if(splitArray?.length){
            dispatch(actionToRemoveUserFromRoom(splitArray[0]))
        }
  ...
  const handleUnmute = useCallback(
    (p) => {
      if (!callFrame) return;
      console.log("UNMUTING",callFrame);
      if (p?.user_id === "local") {
