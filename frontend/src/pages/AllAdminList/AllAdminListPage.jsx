...
const AllAdminListPage = () => {
    const allUsersDataArray = useSelector((state) => state.allAdminUsersDataArray);
 ...
 const deleteUserData = ()=>{
  ...
  const showUserProfilePage = (key) =>{
      history.push('/app/admin-member-profile/'+key);
  }
    ...
    return(
        <div className="col-6 pr-2">
          <div className="widget-bg">
              ...
              <div onClick={()=>showUserProfilePage(key)} className="user-role-all-users-count">All users :<strong>({user?.all_members?.length})</strong></div>
          </div>
        </div>
    )
  }
  ...
  const renderAllUserHtml = (users) =>{
    ...
    const callFunctionToSetIsAdmin=(value)=>{
        setIsAdmin(value);
        if(Number(value) !== 1){
            setIsSuperAdmin(value);
        }
    }
    const callFunctionToSetIsSuperAdmin=(value)=>{
        setIsSuperAdmin(value);
...
export default AllAdminListPage;
