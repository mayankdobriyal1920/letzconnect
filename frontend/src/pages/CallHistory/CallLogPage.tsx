...
const CallLogPage = () => {
    ...
    console.log('allCallLogData',allCallLogData)
    ...
    const renderAllCalLogData = (index:any) => {
        console.log(allCallLogData);
        const callLog = allCallLogData[index];
        if(callLog && callLog.members != undefined && callLog.members && callLog.members != '' && jsonTestedString(callLog.members).length) {
            return(
                <div onClick={()=>openCallLogData(index)} className="row">
            ...
            <IonContent fullscreen>
                <div className="chat-list-col">
                    <div className='container call_log_page_container'>
                        <Virtuoso
                            totalCount={allCallLogData.length}
                            className="vertual_list_scroll_booking_page"
                            itemContent={(index) => renderAllCalLogData(index)}
                        />
                    </div>
                </div>
            </IonContent>
